...
class _AddNewAddressScreenState extends State<AddNewAddressScreen> {
  ...
  Future<void> _goToCurrentLocation() async {
    try {
      ...
      if (permission == LocationPermission.denied ||
        ...
        if (requested == LocationPermission.denied ||
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('يرجى تفعيل صلاحية الموقع أولاً')),
          );
          return;
        }
      }
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تعذر جلب موقعك الحالي')),
      );
    }
  }
                ...
                children: [
                  if (!_locationSelected)
                    const Row(
                      children: [
                        ...
                        Expanded(
                            child: Text(
                                'يرجى تحديد الموقع على الخريطة قبل الحفظ',
                                style: TextStyle(
                                    color: Colors.orange,
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'اسم العنوان (مثلاً الفرع الرئيسي)',
                      filled: true,
