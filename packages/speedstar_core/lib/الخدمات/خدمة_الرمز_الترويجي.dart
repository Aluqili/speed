/// خدمة الرمز الترويجي: إضافة، تحقق، وتحديث الاستخدام.
import 'package:cloud_firestore/cloud_firestore.dart';

class PromocodeServiceArabic {
  final _col = FirebaseFirestore.instance.collection('promocodes');

  /// إضافة رمز ترويجي جديد (لوحة الأدمن)
  ...
  Future<Map<String, dynamic>?> validatePromocode({
    ...
    // تفعيل وتاريخ الانتهاء
    if (!(promo['isActive'] ?? false)) return null;
    final expiry = promo['expiryDate'];
    if (expiry != null &&
        expiry is Timestamp &&
        expiry.toDate().isBefore(DateTime.now())) return null;

    // المطعم المستهدف
    if (promo['restaurantId'] != null &&
        promo['restaurantId'] != '' &&
        promo['restaurantId'] != restaurantId) return null;

    // الحد الأدنى للطلب
    if (promo['minOrder'] != null && orderTotal < promo['minOrder'])
      return null;

    // الحد الأعلى للاستخدام الكلي
    if (promo['maxUsage'] != null &&
        promo['usedCount'] != null &&
        promo['usedCount'] >= promo['maxUsage']) return null;

    // الحد الأعلى لكل مستخدم
